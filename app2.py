...
def store_arxiv_in_vector_store(arxiv_papers):
    embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    arxiv_texts = [p["summary"] for p in arxiv_papers]
    if arxiv_texts:
        vector_store = FAISS.from_texts(arxiv_texts, embedding=embeddings)
...
def store_text_in_vector_store(text_chunks):
    embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    if text_chunks:
...
def search_vector_store(query):
    ...
    try:
        ...
        return new_db.similarity_search(query, k=3)
    except:
        return []  # Return empty if no vector store found
...
def get_conversational_chain():
    ...
    model = ChatGoogleGenerativeAI(model="gemini-2.0-flash", temperature=0.2)  # Lower temp to reduce hallucination
...
def user_input(user_question):
    ...
    if not docs:
        st.write("⚠️ No relevant information found in stored PDFs or arXiv papers.")
        return
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Process PDFs"):
            with st.spinner("Processing..."):
                ...
                store_text_in_vector_store(text_chunks)
        ...
        if st.button("🔍 Fetch Papers"):
            with st.spinner("Fetching papers..."):
                ...
                store_arxiv_in_vector_store(papers)  # Store in FAISS
                for paper in papers:
